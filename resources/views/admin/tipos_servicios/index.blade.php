<x-app-layout>
    <x-slot name="header">
        <h2>Configuración · Gestor de Servicios</h2>
    </x-slot>
    ...
    <div class="info-banner">
        ...
        <span>Los servicios que crees aquí alimentan automáticamente el selector <strong>«Servicio»</strong> al registrar un <a href="{{ route('admin.dispensacion.pacientes.create') }}" style="color:var(--inst); font-weight:700;">nuevo paciente</a>. El <strong>Código</strong> se asigna solo y la <strong>Observación</strong> describe el área o servicio hospitalario.</span>
    </div>

    <div class="dash-grid">
        <div class="dash-card blue">
            <div><div class="label">Servicios registrados</div><div class="value">{{ $stats['total'] }}</div></div>
            <div class="ic">🏥</div>
        </div>
    ...
    <div class="layout-catalog">
        <aside>
            <div class="filter-card">
                <h3>Filtros</h3>
                <form method="GET" action="{{ route('admin.tipos_servicios.index') }}">
                    <div class="filter-group">
                        <label>Buscar (detalle u observación)</label>
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Ej: UCI, Urgencias...">
                    </div>
                    <div class="filter-group">
                    ...
                    <div class="filter-actions">
                        ...
                        <a href="{{ route('admin.tipos_servicios.index') }}" class="btn-outline" style="text-align:center;">Restablecer</a>
                    </div>
                </form>
            </div>
        </aside>
        ...
        <main>
            <div class="catalog-top">
                <div>
                    ...
                    <p>Administra los servicios o áreas hospitalarias donde se ubica a los pacientes. Crea, edita o elimina según las necesidades del servicio farmacéutico.</p>
                </div>
                @puede('Gestor de Servicios','Crear')
            ...
            <div class="card-grid">
                @forelse($tipos as $tipo)
                    @php $uso = $usoPorId[$tipo->id] ?? 0; @endphp
                    <div class="te-card">
                        ...
                        <div class="te-foot">
                            @if($uso > 0)
                                <span class="badge-uso uso-yes">● {{ $uso }} paciente{{ $uso == 1 ? '' : 's' }}</span>
                            @else
                                <span class="badge-uso uso-no">● Sin uso</span>
                            @endif
                            <div style="display:flex; gap:6px;">
                                @puede('Gestor de Servicios','Editar')
                                ...
                                @endpuede
                                @puede('Gestor de Servicios','Eliminar')
                                <form action="{{ route('admin.tipos_servicios.destroy', $tipo) }}" method="POST"
                                      onsubmit="return confirm('¿Eliminar el servicio «{{ $tipo->Detalle }}»? Esta acción no se puede deshacer.')" style="display:inline;">
                                    @csrf @method('DELETE')
                ...
                @empty
                    <div class="empty-state">
                        <h3 style="font-size:1.2rem; color:#4b5563;">Sin servicios</h3>
                        <p style="color:#6b7280;">No se encontraron resultados con los filtros aplicados. Crea el primero con el botón «Nuevo servicio».</p>
                    </div>
                @endforelse
            </div>
    ...
    <div id="modal-tipo" class="modal-overlay">
        <div class="modal-box">
            ...
            <form id="form-tipo" action="{{ route('admin.tipos_servicios.store') }}" method="POST">
                @csrf
                <input type="hidden" name="_method" id="form-method" value="POST">

                <div class="modal-body">
                    <div class="form-group">
                        <label>Código <span class="hint">(se asigna automáticamente)</span></label>
                        <input type="text" id="f_codigo" value="Automático" readonly>
                    </div>

                    <div class="form-group">
                        <label>Detalle <span class="hint">— nombre del servicio *</span></label>
                        <input type="text" name="Detalle" id="f_detalle" maxlength="120" required
                               placeholder="Ej: UCI Adultos, Urgencias, Hospitalización..."
                               value="{{ old('Detalle') }}">
                    </div>

                    <div class="form-group">
                        <label>Observación <span class="hint">— ¿con qué objetivo se crea este servicio?</span></label>
                        <textarea name="Observacion" id="f_observacion" rows="4" maxlength="300"
                                  placeholder="Describe el área o servicio hospitalario..."
    ...
    <script>
        ...
        function resetForm() {
            ...
            f.reset();
            f.action = "{{ route('admin.tipos_servicios.store') }}";
        ...
        function editTipo(t) {
            ...
            document.getElementById('form-tipo').action = "{{ url('admin/tipos-servicios') }}/" + t.id;
