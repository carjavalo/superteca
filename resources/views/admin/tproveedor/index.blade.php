<x-app-layout>
    <x-slot name="header">
        <h2>Configuración · Gestor Tipo de Proveedores</h2>
    </x-slot>
    ...
    <div class="info-banner">
        ...
        <span>Los tipos que crees aquí alimentan automáticamente el selector <strong>«Tipo de proveedor»</strong> del modal <strong>Nuevo Proveedor</strong> en <a href="{{ route('admin.proveedores.index') }}" style="color:var(--inst); font-weight:700;">Proveedores</a> y en cualquier otra vista donde se diligencie. El <strong>Código</strong> se asigna solo y la <strong>Observación</strong> explica para qué sirve cada tipo.</span>
    </div>
    ...
    <div class="dash-grid">
        ...
        <div class="dash-card">
            <div><div class="label">Proveedores asociados</div><div class="value">{{ number_format($stats['proveedores']) }}</div></div>
            <div class="ic">🚚</div>
        </div>
    </div>
    ...
    <div class="layout-catalog">
        <aside>
            <div class="filter-card">
                <h3>Filtros</h3>
                <form method="GET" action="{{ route('admin.tproveedor.index') }}">
                    <div class="filter-group">
                        <label>Buscar (detalle u observación)</label>
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Ej: Distribuidor, Laboratorio...">
                    </div>
                    <div class="filter-group">
                    ...
                    <div class="filter-actions">
                        ...
                        <a href="{{ route('admin.tproveedor.index') }}" class="btn-outline" style="text-align:center;">Restablecer</a>
                    </div>
                </form>
            </div>
        </aside>
        ...
        <main>
            <div class="catalog-top">
                <div>
                    <h2>Catálogo de Tipos de Proveedor</h2>
                    <p>Administra los tipos que clasifican a los proveedores del servicio farmacéutico. Crea, edita o elimina según las necesidades.</p>
                </div>
                @puede('Gestor Tipo de Proveedores','Crear')
            ...
            <div class="card-grid">
                @forelse($tipos as $tipo)
                    ...
                    <div class="te-card">
                        ...
                        <div class="te-foot">
                            @if($uso > 0)
                                <span class="badge-uso uso-yes">● {{ $uso }} proveedor{{ $uso == 1 ? '' : 'es' }}</span>
                            @else
                                <span class="badge-uso uso-no">● Sin uso</span>
                            @endif
                            <div style="display:flex; gap:6px;">
                                @puede('Gestor Tipo de Proveedores','Editar')
                                ...
                                @endpuede
                                @puede('Gestor Tipo de Proveedores','Eliminar')
                                <form action="{{ route('admin.tproveedor.destroy', $tipo) }}" method="POST"
                                      onsubmit="return confirm('¿Eliminar el tipo de proveedor «{{ $tipo->Detalle }}»? Esta acción no se puede deshacer.')" style="display:inline;">
                                    @csrf @method('DELETE')
                ...
                @empty
                    <div class="empty-state">
                        <h3 style="font-size:1.2rem; color:#4b5563;">Sin tipos de proveedor</h3>
                        <p style="color:#6b7280;">No se encontraron resultados con los filtros aplicados. Crea el primero con el botón «Nuevo tipo».</p>
                    </div>
                @endforelse
            </div>
    ...
    <div id="modal-tipo" class="modal-overlay">
        <div class="modal-box">
            <div class="modal-header">
                <h3 id="modal-title">Nuevo tipo de proveedor</h3>
                <button type="button" class="modal-close" onclick="closeModal()">&times;</button>
            </div>
            <form id="form-tipo" action="{{ route('admin.tproveedor.store') }}" method="POST">
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group" style="margin-top:16px;">
                        <label>Detalle <span class="hint">— nombre del tipo de proveedor *</span></label>
                        <input type="text" name="Detalle" id="f_detalle" maxlength="120" required
                               placeholder="Ej: Distribuidor, Laboratorio, Droguería..."
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="Observacion" id="f_observacion" rows="4" maxlength="300"
                                  placeholder="Explica para qué se utiliza este tipo de proveedor..."
    ...
    <script>
        ...
        function resetForm() {
            ...
            f.reset();
            f.action = "{{ route('admin.tproveedor.store') }}";
            document.getElementById('form-method').value = 'POST';
            document.getElementById('modal-title').textContent = 'Nuevo tipo de proveedor';
        ...
        function editTipo(t) {
            resetForm();
            document.getElementById('modal-title').textContent = 'Editar tipo de proveedor';
            document.getElementById('form-tipo').action = "{{ url('admin/tproveedor') }}/" + t.id;
