<x-app-layout>
    <x-slot name="header">
        <h2>Configuración · Gestor de Tipos de Administración</h2>
    </x-slot>
    ...
    <div class="info-banner">
        ...
        <span>Los tipos que crees aquí alimentan automáticamente el selector <strong>«Tipo»</strong> del modal <strong>Nueva Vía de Administración</strong> en <a href="{{ route('admin.vias_administracion.index') }}" style="color:var(--inst); font-weight:700;">Vías de Administración</a>. El <strong>Código</strong> se asigna solo y la <strong>Observación</strong> explica para qué sirve cada tipo.</span>
    </div>
    ...
    <div class="dash-grid">
        ...
        <div class="dash-card">
            <div><div class="label">Vías asociadas</div><div class="value">{{ number_format($stats['vias']) }}</div></div>
            <div class="ic">💉</div>
        </div>
    </div>
    ...
    <div class="layout-catalog">
        <aside>
            <div class="filter-card">
                <h3>Filtros</h3>
                <form method="GET" action="{{ route('admin.tipos_administracion.index') }}">
                    <div class="filter-group">
                        <label>Buscar (detalle u observación)</label>
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Ej: Parenteral, Tópica...">
                    </div>
                    <div class="filter-group">
                    ...
                    <div class="filter-actions">
                        ...
                        <a href="{{ route('admin.tipos_administracion.index') }}" class="btn-outline" style="text-align:center;">Restablecer</a>
                    </div>
                </form>
            </div>
        </aside>
        ...
        <main>
            <div class="catalog-top">
                <div>
                    <h2>Catálogo de Tipos de Administración</h2>
                    <p>Administra los tipos (vías) que clasifican la administración de medicamentos. Crea, edita o elimina según las necesidades del servicio farmacéutico.</p>
                </div>
                @puede('Gestor Tipos de Administración','Crear')
            ...
            <div class="card-grid">
                @forelse($tipos as $tipo)
                    ...
                    <div class="te-card">
                        ...
                        <div class="te-foot">
                            @if($uso > 0)
                                <span class="badge-uso uso-yes">● {{ $uso }} vía{{ $uso == 1 ? '' : 's' }}</span>
                            @else
                                <span class="badge-uso uso-no">● Sin uso</span>
                            @endif
                            <div style="display:flex; gap:6px;">
                                @puede('Gestor Tipos de Administración','Editar')
                                ...
                                @endpuede
                                @puede('Gestor Tipos de Administración','Eliminar')
                                <form action="{{ route('admin.tipos_administracion.destroy', $tipo) }}" method="POST"
                                      onsubmit="return confirm('¿Eliminar el tipo de administración «{{ $tipo->Detalle }}»? Esta acción no se puede deshacer.')" style="display:inline;">
                                    @csrf @method('DELETE')
                ...
                @empty
                    <div class="empty-state">
                        <h3 style="font-size:1.2rem; color:#4b5563;">Sin tipos de administración</h3>
    ...
    <div id="modal-tipo" class="modal-overlay">
        <div class="modal-box">
            <div class="modal-header">
                <h3 id="modal-title">Nuevo tipo de administración</h3>
                <button type="button" class="modal-close" onclick="closeModal()">&times;</button>
            </div>
            <form id="form-tipo" action="{{ route('admin.tipos_administracion.store') }}" method="POST">
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group" style="margin-top:16px;">
                        <label>Detalle <span class="hint">— nombre del tipo de administración *</span></label>
                        <input type="text" name="Detalle" id="f_detalle" maxlength="120" required
                               placeholder="Ej: Parenteral, Enteral, Tópica..."
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="Observacion" id="f_observacion" rows="4" maxlength="300"
                                  placeholder="Explica para qué se utiliza este tipo de administración..."
    ...
    <script>
        ...
        function resetForm() {
            ...
            f.reset();
            f.action = "{{ route('admin.tipos_administracion.store') }}";
            document.getElementById('form-method').value = 'POST';
            document.getElementById('modal-title').textContent = 'Nuevo tipo de administración';
        ...
        function editTipo(t) {
            resetForm();
            document.getElementById('modal-title').textContent = 'Editar tipo de administración';
            document.getElementById('form-tipo').action = "{{ url('admin/tipos-administracion') }}/" + t.id;
