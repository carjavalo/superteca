...
/**
 * Catálogo dinámico de Tipos de Proveedor.
 *
 * Alimenta el campo «Tipo de proveedor» del modal Nuevo Proveedor. El valor que
 * se guarda en `proveedores.tipo_proveedor` es el «código» derivado del Detalle
 * (mayúsculas, sin acentos). Así, los 6 tipos sembrados coinciden exactamente
 * con las claves históricas (DISTRIBUIDOR, LABORATORIO, …, SERVICIOS) y no se
 * rompe la data ya registrada ni las etiquetas existentes.
 */
class TProveedor extends Model
{
    ...
    /** Código interno derivado del Detalle (lo que se persiste en proveedores.tipo_proveedor). */
     ...
     * de proveedores sin generar consultas N+1 al listar.
     */
    ...
    /**
     * Códigos válidos para validar proveedores.tipo_proveedor: los del catálogo
     * dinámico más las claves legadas de Proveedor::TIPOS.
     */
    public static function codigosDisponibles(): array
    {
        $delCatalogo = array_keys(static::mapaCodigoDetalle());
        $legados     = array_keys(Proveedor::TIPOS);
